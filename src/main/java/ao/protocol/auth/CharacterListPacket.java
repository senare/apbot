...
public class CharacterListPacket extends Packet {
	...
	public CharacterListPacket(IoBuffer buff) throws MalformedPacketException {
		...
		for (int i = 0; i < names.length; i++) {
			byte[] name = new byte[buff.getShort()];
			buff.get(name);
			try {
				names[i] = new String(name, "UTF-8");
	...
	public CharacterListPacket(CharacterInfo[] characters) {
		if (characters == null) {
			throw new NullPointerException("No list of characters was passed");
		}
	 ...
	 *
	 * @param data
	 *            the binary data of this packet (without the type and length
	 *            bytes)
	 * @throws NullPointerException
	 *             if data is null
	...
	public CharacterListPacket(byte[] data) throws MalformedPacketException {
		...
		try {
			...
			throw new MalformedPacketException("The packet could not be parsed.", e, new UnparsablePacket(TYPE, data, Direction.TO_CLIENT));
	...
	/**
	 * Returns the character with the given ID or returns null if the character
	 * cannot be found.
	 */
	...
	/**
	 * Returns the character with the given name or returns null if the
