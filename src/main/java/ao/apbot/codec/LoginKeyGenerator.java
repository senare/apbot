 ...
 * Diffie-Hellman Key Exchange, Tiny Encryption Algorithm (TEA), and likely some
 * other unidentified algorithms.
 * </p>
 *
 * <p>
 * <b>References:</b>
 * <ul>
 * <li><a href="http://en.wikipedia.org/wiki/Diffie-Hellman_key_exchange"
 * target="_blank">Diffie-Hellman Key Exchange</a></li>
 * <li><a href="http://en.wikipedia.org/wiki/Tiny_Encryption_Algorithm"
 * target="_blank">Tiny Encryption Algorithm (TEA)</a></li>
 * <li><a href="http://boards.hackersquest.org/viewtopic.php?p=33507"
 * target="_blank">Class ao.f from JavaBot</a></li>
 * <li><a href="http://sourceforge.net/projects/aochat" target="_blank">Class
 * aochat.loginMessage from a J++ based bot library written by migisan and
 * quaseem</a></li>
 * <li><a href="http://kiwu.org/aochat/" target="_blank">Script key.py from a
 * Python port of AOChat.php</a></li>
 * <li><a href="http://auno.org/dev/aochat/AOChat.php"
 * target="_blank">AOChat.php</a></li>
 * </ul>
 * </p>
 *
...
public class LoginKeyGenerator {
	...
	/**
	 * A constant that is sent to the AO server when attempting to authenticate
	 * a bot/user.
	 * 
	 * @see ao.protocol.auth.LoginRequestPacket
	 */
	...
	/**
	 * See <a
	 * href="http://en.wikipedia.org/wiki/Diffie-Hellman_key_exchange">Diffie
	 * -Hellman Key Exchange</a>
	 */
	...
	/**
	 * Given a server seed and a user name and password pair, this function
	 * generates an encrypted login key.
	 *
	 * @param serverSeed
	 *            the seed that the AO server provided when the connection to it
	 *            was first opened (this is currently a 32 character hexadecimal
	 *            string (128 bit))
	 * @param accountName
	 *            the user name that the bot/user provided
	 * @param password
	 *            the password that the bot/user provided
	 * @return the generated encrypted login key
	...
	public static String generateLoginKey(String serverSeed, String accountName, String password) {
		if (serverSeed == null) {
			throw new NullPointerException("No server seed was given.");
		} else if (accountName == null) {
			throw new NullPointerException("No user name was given.");
		} else if (password == null) {
			throw new NullPointerException("No password was given.");
		} else {
	 ...
	 *            string (128 bit))
	 * @param localSeed
	 *            a random 16 character hexadecimal string (64 bit) generated by
	 *            the client
	 * @param accountName
	 *            the user name that the bot/user provided
	 * @param password
	...
	/**
	 * Generates a random local seed. The seed generated is a {@code length}
	 * character hexadecimal string ({@code length*4} bit).
	 *
	 * @param length
	 *            the number of characters in the generated seed
	 * @return a random {@code length} character hexadecimal string (
	 *         {@code length*4} bit)
	 *
	...
	/**
	 * Truncates or zero fills the encryption key so that it is always
	 * {@code length} characters long. If {@code key.length()} is greater than
	 * {@code length}, then the first {@code length} characters of the key are
	 * returned. If <code>key.length()</code> is less than {@code length}, then
	 * {@code length - key.length()} zeros are inserted at the begining of the
	 * key and the result is returned.
	 *
	 * @param key
	 *            the encryption key that will be truncated or zero filled
	 * @param length
	 *            the length of the returned encryption key
	 * @return the truncated or zero-filled encryption key
	 *
	 * @see #generateLoginKey(String, String, String, String)
	...
	/**
	 * Encrypts a bot/user's login data using a 128 bit encryption key (passed
	 ...
	 * @return the encrypted login data
	 */
	public static String encrypt(String encryptionKey, String loginData) {
		if (encryptionKey.length() < 32) {
			throw new IllegalArgumentException("The encryption key is too short.");
		}
		...
		for (int i = 0; i < unencryptedInts.length; i += 2) {
			...
			encrypted += intArrayToHexString(newBlock);
		} // end for
	 ...
	 * @return a random sequence of {@code length} characters
	 *
	 * @see #encrypt(String, String)
	 */
	 ...
	 * @return a four character string created from the given integer's binary
	 *         data
	 *
	 * @see #encrypt(String, String)
	 */
	...
	/**
	 * Creates a sequence of ' ' characters to append to the unencrypted login
	 * data, so that the length of the unecrypted login data is evenly divisible
	 * by 8.
	 *
	 * @param totalLength
	...
	/**
	 * Turns a hexadecimal string into a sequence of integers. This function is
	 * used to turn the 128 bit encryption key from a hexadecimal string into a
	 * sequence of four integers to encrypt the login data.
	 *
	 * @param key
	 *            the hexadecimal string that will be turned into a sequence of
	 *            integers
	 * @return the sequence of integers created from the hexadecimal string
	 */
	...
	/**
	 * Turns a sequence of characters into a sequence of integers. This function
	 * is used to turn the unencrypted login data from a sequence of characters
	 * to a sequence of integers; so that it can be encrypted.
	 *
	 * @param s
	 *            the sequence of characters that will be turned into a sequence
	 *            of integers
	 * @return the sequence of integers created from the sequence of characters
	 */
	 ...
	 *
	 * @param block
	 *            the 64 bit block that will be used to create the hexadecimal
	 *            string
	 * @return a hexadecimal string created from the block
	 */
	public static String intArrayToHexString(int[] block) {
		String output = "";
		for (int i = 0; i < block.length; i++) {
			output += String.format("%02x%02x%02x%02x", block[i] & 0xff, block[i] >> 8 & 0xff, block[i] >> 16 & 0xff, block[i] >> 24 & 0xff);
		}
		return output;
	}
